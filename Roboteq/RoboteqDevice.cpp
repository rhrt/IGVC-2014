...
int RoboteqDevice::Connect(string port)
{
	...
	status = IssueCommand("?", "LK", 20, response);
...
int RoboteqDevice::SetCommand(string commandItem, int index, int value)
{
	string response;
	string command;
	char args[50];
	...
	if(commandItem < 0 || commandItem > 255)
		return RQ_INVALID_COMMAND_ITEM;
	sprintf(command, "$%s", commandItem);
	*/
	command.append(commandItem);
	sprintf(args, "%i %i", index, value);
	if(index == MISSING_VALUE)
	{
		if(value != MISSING_VALUE)
